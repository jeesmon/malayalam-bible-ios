...
//  Copyright 2008 __iEnterprises__.  All rights reserved.  Maintained by __Notetech__.
//
...
+ (void)setPathToRoundedRect:(CGRect)rect forInset:(NSUInteger)inset inContext:(CGContextRef)context
{
	...
	static const NSUInteger cornerRadius = 0;//+20110505
...
+ (void)drawGlossyRect:(CGRect)rect withColor:(UIColor*)color inContext:(CGContextRef)context
{
	...
	static const float EXP_COEFFICIENT	= 1.0;
	static const float REFLECTION_MAX	= 1.0;
	static const float REFLECTION_MIN	= 0;
...
- (UIButton*)shinyButton:(UIButton *)button WithWidth:(NSUInteger)width height:(NSUInteger)height color:(UIColor*)color color2:(UIColor *)color2
{
	...
	[button setBackgroundToGlossyRectOfColor:color withBorder:NO forState:UIControlStateNormal];
	//[button setBackgroundToGlossyRectOfColor:color2 withBorder:YES forState:UIControlStateHighlighted];
	...
	//[button setTitleShadowOffset:CGSizeMake(0, -1)];//commentes for os 3.0
	//[button setFont:[UIFont boldSystemFontOfSize:17]];
